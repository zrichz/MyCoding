...
def create_noise_wavetable():
    ...
    try:
        print("Available noise types: white, pink, brown, filtered")
        noise_input = input("Enter noise types to include (e.g., white pink): ").strip().lower().split()
        
        noise_types = []
        for noise in noise_input:
            if noise in ['white', 'pink', 'brown', 'filtered']:
                noise_types.append(noise)
...
def create_wavetable(wavetable_type):
    ...
    if wavetable_type == 1:
        ...
        print("Available chaotic equations: logistic, henon, lorenz")
        eq_type = input("Enter equation type (default: logistic): ").lower()
        if eq_type not in ['logistic', 'henon', 'lorenz']:
            eq_type = 'logistic'
        ...
        print("Available physical models: string, tube, membrane")
        model = input("Enter model type (default: string): ").lower()
        if model not in ['string', 'tube', 'membrane']:
            model = 'string'
        ...
        print("Available fractal types: mandelbrot, julia, ifs")
        fractal_type = input("Enter fractal type (default: mandelbrot): ").lower()
        if fractal_type not in ['mandelbrot', 'julia', 'ifs']:
        ...
        print("Available waveforms: sine, square, sawtooth, triangle")
        waveform = input("Enter base waveform (default: sine): ").lower()
