...
# Add GUI for directory selection
import tkinter as tk
from tkinter import filedialog

def select_directory(prompt="Select a directory"):
    root = tk.Tk()
    root.withdraw()
...
# ----- PARAMETERS -----
# This script v27 uses the Bristow-Johnson cycle extraction technique, which extracts audio cycles based on pitch and phase alignment.
# It ensures seamless frames by reconstructing cycles with minimal discontinuities. For more details, refer to the original paper or documentation.

# Prompt user to select input and output directories via GUI
try:
    print("Please select the input directory containing WAV files.")
    input_directory = select_directory("Select Input Directory")
    if not input_directory:
        raise Exception("Input directory selection cancelled.")
    print("Please select the output directory for saving wavetables.")
    output_directory = select_directory("Select Output Directory")
    if not output_directory:
        raise Exception("Output directory selection cancelled.")
...
num_harmonics = 5 # Number of harmonics to consider for phase diff (values between 3-8 seem pretty good)
